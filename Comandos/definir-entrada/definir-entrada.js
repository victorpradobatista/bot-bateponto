...
module.exports = {
    name: "definir-entrada",
    description: "Setar canal de entrada",
    type: Discord.ApplicationCommandType.ChatInput,
    options: [
        {
          name: 'canal',
          description: 'Selecione um canal de texto para as entradas.',
    ...
    run: async (client, interaction) => {
        ...
        if(interaction.member.permissions.has(Discord.PermissionFlagsBits.KickMembers)){
            ...
            const querySelect = `SELECT * FROM logs WHERE tipo = 'welcome'`
            d.query(querySelect, function(err, result) {
                if(err) throw err;
                if(result == false) {
                    const queryInsert = `INSERT INTO logs (tipo, id) VALUES ('welcome', '${channel.id}')`
                    ...
                    const queryUpdate = `UPDATE logs SET id = '${channel.id}' WHERE tipo = 'welcome'`
                    d.query(queryUpdate)
